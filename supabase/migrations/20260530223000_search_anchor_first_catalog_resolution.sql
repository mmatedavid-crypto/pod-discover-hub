-- Search anchor-first repair: every query should first be allowed to resolve
-- against our catalog anchors. This keeps short/numeric podcast brands such as
-- "444" from being discarded as stopwords and lets podcast slugs participate
-- in known-item matching without hand-maintaining one-off aliases.
...
BEGIN
  ...
  base AS (
    SELECT
      ...
      p.slug,
      p.normalized_title,
      public.normalize_podcast_title(p.slug) AS normalized_slug,
      p.podiverzum_rank,
      p.rank_label
    ...
    WHERE p.is_hungarian = true
      ...
      AND (
        ...
        OR public.normalize_podcast_title(p.slug) = qn_nf
        OR (' ' || p.normalized_title || ' ') LIKE ('% ' || qn_nf || ' %')
        OR p.normalized_title LIKE (qn_nf || '%')
        OR public.normalize_podcast_title(p.slug) LIKE (qn_nf || '%')
    ...
    SELECT b.id, b.title, b.slug, b.podiverzum_rank, b.rank_label,
      GREATEST(
        ...
        CASE
          ...
          WHEN b.normalized_slug = qn_nf THEN 0.99::real
          WHEN is_short AND b.normalized_title LIKE (qn_nf || '%') THEN 0.95::real
          WHEN is_short AND b.normalized_slug LIKE (qn_nf || '%') THEN 0.94::real
          ...
          WHEN b.normalized_slug LIKE (qn_nf || '%') THEN 0.84::real
      ...
      CASE
        ...
        WHEN b.normalized_slug = qn_nf THEN 'slug'
        WHEN is_short AND b.normalized_title LIKE (qn_nf || '%') THEN 'prefix'
        WHEN is_short AND b.normalized_slug LIKE (qn_nf || '%') THEN 'slug_prefix'
        ...
        WHEN b.normalized_slug LIKE (qn_nf || '%') THEN 'slug_prefix'
        ELSE 'trgm'
      END AS mtype
    FROM base b
  )
...
GRANT EXECUTE ON FUNCTION public.match_podcast_by_name(text, integer, double precision)
  TO anon, authenticated, service_role;

UPDATE public.app_settings
SET value = jsonb_set(
              jsonb_set(value, '{ranking_version}', to_jsonb(COALESCE((value->>'ranking_version')::int, 3) + 1)),
              '{understanding_version}', to_jsonb(COALESCE((value->>'understanding_version')::int, 3) + 1)
            ),
    updated_at = now()
