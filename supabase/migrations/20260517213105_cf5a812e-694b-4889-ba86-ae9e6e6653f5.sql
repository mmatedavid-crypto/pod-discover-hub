CREATE OR REPLACE FUNCTION public.get_mood_episode_recommendations(p_mood_slug text, p_limit integer DEFAULT 12, p_exclude uuid[] DEFAULT '{}'::uuid[])
...
DECLARE
  ...
  v_quality numeric;
BEGIN
  -- Widen HNSW search so post-filter (HU-only + healthy RSS) has enough candidates.
  -- Without this, ef_search=40 returns ~40 globally-nearest rows; most are non-HU
  -- and some moods (e.g. "kulturahoz") end up with zero post-filter survivors.
...
SELECT public.recompute_mood_recommended_counts();