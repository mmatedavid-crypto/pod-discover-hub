-- Stronger DB-side guard for public AI text.
-- The first version accepted any Hungarian diacritic, which can let English
-- summaries through when they contain a Hungarian proper noun. This version
-- mirrors the app-side guard more closely: English function words must not
-- dominate unless there is real Hungarian stopword/diacritic density.
...
BEGIN
  ...
  FOREACH w IN ARRAY words LOOP
    IF w = ANY (ARRAY[
      'és','hogy','az','egy','van','nem','mert','podcast','adás','adas','epizód','epizod',
    ...
    ELSIF w = ANY (ARRAY[
      ...
      'conversation','interview','host','guest','listeners'
    ]) THEN
      en_hits := en_hits + 1;
    END IF;
  END LOOP;
...
CREATE TEMP TABLE tmp_non_hu_episode_public_text_v2 AS
...
CREATE TEMP TABLE tmp_non_hu_podcast_public_text_v2 AS
SELECT p.id
...
SET
  ...
  ai_enriched_at = NULL
FROM tmp_non_hu_episode_public_text_v2 bad
WHERE bad.id = e.id;
...
SET
  ...
  ai_enriched_at = NULL
FROM tmp_non_hu_podcast_public_text_v2 bad
WHERE bad.id = p.id;
...
SELECT
  ...
  bad.id,
  100,
  md5('non_hu_public_text_repair_episode_v2:' || bad.id::text),
  'pending',
  jsonb_build_object('reason', 'non_hu_public_text_repair', 'source', 'migration_20260603131000')
FROM tmp_non_hu_episode_public_text_v2 bad
...
SELECT
  ...
  bad.id,
  100,
  md5('non_hu_public_text_repair_podcast_v2:' || bad.id::text),
  'pending',
  jsonb_build_object('reason', 'non_hu_public_text_repair', 'source', 'migration_20260603131000')
FROM tmp_non_hu_podcast_public_text_v2 bad
ON CONFLICT (kind, target_type, target_id, input_hash) DO NOTHING;
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'repair_job_source', 'migration_20260603131000',
    'note', 'Public AI text on Podiverzum must be Hungarian. English-dominant fields are nulled and queued for SEO regeneration.'
  ),
  now()
)
...
DROP TABLE IF EXISTS tmp_non_hu_episode_public_text_v2;
DROP TABLE IF EXISTS tmp_non_hu_podcast_public_text_v2;
