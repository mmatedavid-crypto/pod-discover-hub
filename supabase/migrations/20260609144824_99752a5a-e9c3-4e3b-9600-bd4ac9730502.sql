...
VALUES (
  ...
  jsonb_build_object(
    ...
    'reason_sources', jsonb_build_array('shared_people','shared_companies','shared_topics','strong_clean_text_embedding_similarity','user_history_centroid'),
    'public_surface_locked_until_quality_trusted', true,
    'smart_player_public_enable_requires', jsonb_build_array('related_episode_quality_green','entity_monitoring_benchmark_green','smart_player_recommendation_surface_explicit_approval'),
    'reasserted_by', '20260608009000_reassert_recommendation_diagnostics_policy_v4_final'
  ), now()
)
ON CONFLICT (key) DO UPDATE
SET value = public.app_settings.value || EXCLUDED.value, updated_at = now();