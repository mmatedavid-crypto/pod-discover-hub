...
-- Goal: stop relying on chat-memory for "is it running?" questions.
...
BEGIN
  SELECT value, updated_at INTO ctrl, ctrl_updated
  FROM public.app_settings
  WHERE key = p_controls_key
  LIMIT 1;

  SELECT value, updated_at INTO prog, prog_updated
  FROM public.app_settings
  WHERE key = p_progress_key
  LIMIT 1;
  ...
  INTO cron_job
  FROM cron.job j
  WHERE p_cron_pattern IS NOT NULL
    AND j.jobname ILIKE p_cron_pattern
  ...
  RETURN jsonb_build_object(
    ...
    'enabled', enabled,
    'controls_key', p_controls_key,
    'progress_key', p_progress_key,
    'controls_updated_at', ctrl_updated,
    'progress_updated_at', prog_updated,
    'cron', cron_job,
    'last_cron_run', last_run,
    'backlog', p_backlog,
    'backlog_label', p_backlog_label,
...
AS $$
DECLARE
  v_clean_pending bigint;
  v_best_missing bigint;
  v_youtube_due bigint;
  v_native_transcript bigint;
  v_chunk_missing bigint;
  v_person_pending bigint;
  v_org_public_unreviewed bigint;
  v_search_goldens bigint;
  v_weekly_posts bigint;
  v_items jsonb;
BEGIN
  ...
  WHERE p.is_hungarian = true
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.episode_clean_text ct
  ...
  WHERE p.is_hungarian = true
    ...
    AND COALESCE(e.description, '') <> ''
    AND NOT EXISTS (
      SELECT 1 FROM public.episode_best_text_source b
  ...
  WHERE p.is_hungarian = true
    ...
    AND (
      p.youtube_url IS NOT NULL
      OR EXISTS (
        SELECT 1 FROM public.podcast_youtube_candidates pyc
        WHERE pyc.podcast_id = p.id
          AND pyc.status IN ('confirmed', 'accepted')
      )
    )
    AND (
      p.youtube_last_episode_pair_at IS NULL
      OR p.youtube_last_episode_pair_at < now() - interval '7 days'
    );

  SELECT count(*)
  INTO v_native_transcript
  FROM public.episode_youtube_links yl
  WHERE yl.status = 'confirmed'
    AND yl.youtube_caption_available = true
    AND NOT EXISTS (
      SELECT 1 FROM public.episode_transcripts tr
      WHERE tr.episode_id = yl.episode_id
        AND tr.model IN ('supadata-youtube', 'supadata-youtube-native')
    );

  SELECT count(DISTINCT e.id)
  INTO v_chunk_missing
  FROM public.episodes e
  JOIN public.podcasts p ON p.id = e.podcast_id
  ...
  WHERE p.is_hungarian = true
    ...
    AND ct.cleaner_method = 'deterministic_v4'
    AND NOT EXISTS (
      SELECT 1 FROM public.episode_chunks ch
  ...
  SELECT count(*)
  INTO v_org_public_unreviewed
  FROM public.organizations
  WHERE is_public = true
    AND COALESCE(ai_review_status, 'pending') IN ('pending', 'needs_review');

  SELECT count(*)
  INTO v_search_goldens
  ...
  v_items := jsonb_build_array(
    public.pipeline_health_item_v1(
      'clean_text_v4',
      'clean_text_autopilot',
      'episode_clean_text_candidate_progress',
      '%clean-text%',
      v_clean_pending,
      'HU episodes without deterministic_v4 clean text'
    ),
    public.pipeline_health_item_v1(
      'best_text_source',
      ...
      '%best-text-source%',
      v_best_missing,
      'HU episodes without selected best text source'
    ),
    public.pipeline_health_item_v1(
      'youtube_episode_pairer',
      'youtube_episode_pairer_controls',
      'youtube_episode_pairer_progress',
      '%youtube%pair%',
      v_youtube_due,
      'HU podcasts due for YouTube episode pairing'
    ),
    public.pipeline_health_item_v1(
      'native_transcript_fetch',
      'youtube_transcript_controls',
      'youtube_transcript_progress',
      '%transcript%',
      v_native_transcript,
      'confirmed YouTube episodes with native captions not fetched'
    ),
    public.pipeline_health_item_v1(
      'episode_chunk_embeddings',
      'embed_episode_chunks_controls',
      'embed_episode_chunks_progress',
      '%embed%episode%chunk%',
      v_chunk_missing,
      'clean HU episodes without episode chunks'
    ),
    public.pipeline_health_item_v1(
      'person_relevance_judge',
      'person_relevance_judge_controls',
      'person_relevance_judge_progress',
      '%person%relevance%',
      v_person_pending,
      'person mentions pending relevance judgment'
    ),
    public.pipeline_health_item_v1(
      'entity_quality_autopilot',
      'entity_quality_controls',
      'entity_quality_autopilot_progress',
      '%entity%quality%',
      v_org_public_unreviewed,
      'public organizations pending AI/entity review'
    ),
    public.pipeline_health_item_v1(
      'search_quality_benchmark',
      'search_benchmark_controls',
      'search_benchmark_progress',
      '%search%benchmark%',
      v_search_goldens,
      'active golden queries'
    ),
    public.pipeline_health_item_v1(
      'weekly_editorial',
      'weekly_editorial_controls',
      'weekly_editorial_progress',
      '%weekly%editorial%',
      CASE WHEN v_weekly_posts > 0 THEN 0 ELSE 1 END,
      'weekly editorial posts missing in the last 14 days'
    )
  );
  ...
  RETURN jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      'green', (
        ...
        WHERE x->>'health' = 'green'
      ),
      'yellow', (
        SELECT count(*) FROM jsonb_array_elements(v_items) x
        WHERE x->>'health' = 'yellow'
      ),
      'red', (
        SELECT count(*) FROM jsonb_array_elements(v_items) x
        WHERE x->>'health' = 'red'
