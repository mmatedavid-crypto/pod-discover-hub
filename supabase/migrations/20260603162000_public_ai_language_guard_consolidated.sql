...
SELECT
  ...
  bad.id,
  100,
  md5('non_hu_public_text_repair_episode_v3:' || bad.id::text),
  'pending',
  jsonb_build_object('reason', 'non_hu_public_text_repair', 'source', 'migration_20260603162000')
FROM tmp_non_hu_episode_public_text_v3 bad
...
SELECT
  ...
  bad.id,
  100,
  md5('non_hu_public_text_repair_podcast_v3:' || bad.id::text),
  'pending',
  jsonb_build_object('reason', 'non_hu_public_text_repair', 'source', 'migration_20260603162000')
FROM tmp_non_hu_podcast_public_text_v3 bad
ON CONFLICT (kind, target_type, target_id, input_hash) DO NOTHING;
