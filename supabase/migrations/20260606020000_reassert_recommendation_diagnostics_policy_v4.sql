-- Keep recommendation diagnostics as one cumulative contract.
-- Earlier reassertions added similar_episodes, seed rails, and main rail
-- requirements in separate writes. This version preserves all keys together so
-- later partial policy writes cannot silently erase the public-surface lock or
-- the reason-source contract.
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'reasserted_by', '20260606020000_reassert_recommendation_diagnostics_policy_v4',
    'note', 'Recommendation consumers must emit concise Hungarian related_reason diagnostics and remain publicly locked until quality gates are green.'
  ),
  now()
)
