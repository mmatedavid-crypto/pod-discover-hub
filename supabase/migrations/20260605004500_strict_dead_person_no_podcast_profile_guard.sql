-- A dead or historical entity is not an ordinary podcast-person profile.
-- Name collisions must fail closed until a human explicitly approves an
-- archival/person-page exception.
...
WITH demoted AS (
  ...
  SET
    ...
    editorial_notes = trim(both E'\n' from concat_ws(
      ...
      'strict_dead_person_no_podcast_profile_guard_v3: hidden because deceased/historical/date_of_death rows are not podcast-person profiles without manual archival approval.'
    )),
    updated_at = now()
  WHERE COALESCE(p.manual_approved, false) = false
    ...
    AND (
      ...
      OR COALESCE(p.ai_recommended_action, '') <> 'hide'
    )
  RETURNING p.id
)
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'rule', 'Deceased, historical, date_of_death or is_living=false rows are not public/indexable podcast-person profiles without manual_approved or has_archival_evidence.',
    'name_collision_policy', 'Fail closed: if a living podcaster name was matched to a dead external identity, hide the public person profile until reviewed instead of publishing a wrong biography/page.',
    'updated_at', now()
  ),
  now()
)
