-- merge_organizations(src, dst, note) RPC
-- Moves episode_organization_map, org_ai_review_jobs, organization_aliases from src to dst,
-- preserves wiki/logo fields on dst (only if missing), records src.name as alias on dst,
-- deletes src, logs to canonical_alias_backfill_log, recomputes dst aggregates.
...
BEGIN
  ...
  SELECT name INTO v_dst_name FROM organizations WHERE id = p_dst;
  ...
  -- Preserve src.name as alias on dst (idempotent: skip if already there)
  ...
  VALUES ('org', p_dst, v_src_name, v_src_slug, v_dst_name, NULL, 'merge',
