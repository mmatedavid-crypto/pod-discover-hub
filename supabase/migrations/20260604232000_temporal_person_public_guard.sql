-- Historical/deceased figures are useful episode subjects, but they are not
-- public "podcast person" profiles unless editorially approved as archival.
...
WITH demoted AS (
  ...
  SET
    ...
    browsable_reason = 'temporal_topic_only_guard_v1',
    editorial_notes = concat_ws(
      E'\n',
      nullif(p.editorial_notes, ''),
      'temporal_topic_only_guard_v1: hidden because the entity is deceased/historical/topic-only and has no editorial archival approval.'
    ),
    updated_at = now()
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'rule', 'Deceased or historical people are not public/indexable podcast-person profiles without manual_approved or has_archival_evidence.',
    'death_date_guard', 'date_of_death/is_living=false demotes only when host+guest+participant evidence is zero, to avoid bad external identity matches hiding real living podcasters.'
  ),
  now()
)
