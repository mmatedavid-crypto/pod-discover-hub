-- Keep the public people hub from surfacing weak, hidden or ambiguous
-- identities. The UI also has a client-side guard, but the source RPCs should
-- be safe by contract because these pages are important SEO entry points.
...
AS $$
  WITH base AS (
    ...
    WHERE p.is_public = true
      ...
      AND NOT (
        COALESCE(p.identity_ambiguous, false) = true
        AND COALESCE(p.manual_approved, false) = false
        AND NOT (
          p.wikipedia_match_status = 'verified'
          AND COALESCE(p.wikipedia_match_confidence, 0) >= 0.8
        )
      )
  )
...
AS $function$
  WITH filtered AS (
    ...
    WHERE p.is_public = true
      ...
      AND (
        p_letter IS NULL
        OR (p_letter = '#' AND NOT (upper(unaccent(left(p.name,1))) ~ '^[A-Z]$'))
