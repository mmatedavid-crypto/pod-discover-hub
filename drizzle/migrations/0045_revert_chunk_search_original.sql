DROP FUNCTION public.search_episode_chunks(vector, integer, integer);
CREATE FUNCTION public.search_episode_chunks(query_embedding vector, match_count integer DEFAULT 30, candidate_pool integer DEFAULT 400)
...
AS $function$
  WITH cand AS (
    ...
    SELECT episode_id, sim, chunk_idx, content_snippet, timestamp_start_seconds, timestamp_end_seconds, segment_start_idx, segment_end_idx, source_transcript_model, chunking_method,
      ROW_NUMBER() OVER (PARTITION BY episode_id ORDER BY sim DESC) AS rn FROM cand
  )
  SELECT episode_id, sim AS similarity, 'chunk'::text AS best_source, chunk_idx, content_snippet, timestamp_start_seconds, timestamp_end_seconds, segment_start_idx, segment_end_idx, source_transcript_model, chunking_method
  FROM ranked WHERE rn = 1 ORDER BY sim DESC LIMIT GREATEST(1, LEAST(match_count, 100));
$function$;
REVOKE ALL ON FUNCTION public.search_episode_chunks(vector, integer, integer) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.search_episode_chunks(vector, integer, integer) TO service_role, readonly_codex;
