...
describe("production policy static guards", () => {
  ...
  it("keeps search golden refresh and benchmark on a weekly automated quality loop", () => {
    ...
    expect(verifier).toContain("search_engine_understanding_version_v4");
  ...
  it("keeps production verifier covering recommendation and people identity policies", () => {
    ...
    expect(homeRails).toContain("function filterExplainableRecommendations");
    expect(homeRails).toContain("type DropReason = \"low_similarity\" | \"missing_related_reason\"");
    expect(homeRails).toContain("type RecommendationDiagnostics");
    expect(homeRails).toContain("type PersonalizedRail");
    expect(homeRails).toContain("const rails: PersonalizedRail[] = []");
    expect(homeRails).toContain("function recommendationDiagnostics");
    expect(homeRails).toContain("candidate_count: rows.length");
    expect(homeRails).toContain("dropped.low_similarity++");
    expect(homeRails).toContain("dropped.missing_related_reason++");
    expect(homeRails).toContain("mainDiagnostics");
    expect(homeRails).toContain("seedDiagnostics");
    expect(homeRails).toContain("returned_seed_rail_count: rails.length");
