...
function EntityPill({ item }: { item: EntityCount }) {
  ...
  return (
    <Link
      ...
      className="group inline-flex min-w-0 items-center justify-between gap-2 rounded-lg border border-border/70 bg-card/70 px-3 py-2 text-sm transition-colors hover:border-primary/50 hover:bg-card"
    >
    ...
    <section>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        {lanes.map((lane) => {
          ...
          return (
            <div key={lane.title} className="rounded-lg border border-border/70 bg-card/50 p-3">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide lg:grid lg:grid-cols-1 lg:overflow-visible lg:pb-0">
                {lane.items.slice(0, 6).map((item) => (
                  <div key={`${item.kind}-${item.slug}`} className="min-w-[150px] lg:min-w-0">
                    <EntityPill item={item} />
                  </div>
                ))}
              </div>
            </div>
