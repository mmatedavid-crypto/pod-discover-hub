  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (ents.length) {
        for (const { kind, v } of ents.slice(0, 8)) {
          ...
          const { data: rs } = await supabase
            .from("episodes")
            .select("id,title,slug,published_at,summary,description,audio_url,episode_rank,topics,podcasts!inner(slug,title,image_url,category,podiverzum_rank,rss_status)")
            .neq("id", e.id)
            .contains(col, [v])
      ...
      const { data: mp } = await supabase
        .from("episodes")
        .select("id,title,slug,published_at,summary,description,audio_url,episode_rank,topics,podcasts!inner(slug,title,image_url,category,podiverzum_rank)")
    ...
    <Layout>
      <div className="container mx-auto py-10 max-w-3xl">
        ...
        <div className="text-sm text-muted-foreground mt-1 flex flex-wrap gap-x-3 gap-y-1 items-center">
          ...
          {typeof p.podiverzum_rank === "number" && p.podiverzum_rank > 0 && <span className="text-[10px]">/ Pod {p.podiverzum_rank}</span>}
        </div>
