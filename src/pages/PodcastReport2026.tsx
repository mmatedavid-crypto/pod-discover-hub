      ...
      <article className="mx-auto max-w-3xl px-4 py-8 md:py-12">
        ...
        <section className="mb-12">
          ...
          {(() => {
            ...
            const maxParty = Math.max(...partyMonthly.flatMap((r) => [r.fidesz, r.tisza]));
            ...
            return (
              <div className="mt-6 rounded-lg border border-border bg-card p-5">
                ...
                <p className="text-xs text-muted-foreground mb-4">Olyan magyar podcast-epizódok száma havonta, amelyekben az adott pártot a kanonikus szervezet-adatbázisunk azonosítja (2025. jún. – 2026. máj.).</p>

                <div className="flex items-end gap-1.5 h-44 mb-2">
                  {partyMonthly.map((r) => {
                    const mm = r.m.slice(5);
                    const yy = r.m.slice(2, 4);
                    return (
                      <div key={r.m} className="flex-1 flex flex-col items-center gap-1">
                        <div className="flex items-end gap-0.5 w-full h-full">
                          <div
                            className="flex-1 bg-primary/80 rounded-t relative group"
                            style={{ height: `${(r.fidesz / maxParty) * 100}%` }}
                            title={`Fidesz · ${r.m}: ${r.fidesz} ep`}
                          >
                            <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-[9px] font-mono text-muted-foreground opacity-0 group-hover:opacity-100">{r.fidesz}</div>
                          </div>
                          <div
                            className="flex-1 bg-accent/80 rounded-t relative group"
                            style={{ height: `${(r.tisza / maxParty) * 100}%` }}
                            title={`Tisza · ${r.m}: ${r.tisza} ep`}
                          >
                            <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-[9px] font-mono text-muted-foreground opacity-0 group-hover:opacity-100">{r.tisza}</div>
                          </div>
                        </div>
                        <div className="text-[9px] text-muted-foreground text-center leading-tight">
                          <div>{labelMap[mm]}</div>
                          <div className="opacity-60">'{yy}</div>
                        </div>
                      </div>
                    );
                  })}
