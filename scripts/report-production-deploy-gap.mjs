...
function makeLovablePrompt(plan, groups, unmappedFailures) {
  ...
  if (plan.workers.length) {
    ...
    lines.push(
      ...
      "- legacy category/entity aliases (/category, /topic, /tema, /person, /szemely, /company, /szervezetek, /entitasok, /cegek, /partok, /ingredient) return 301 to their canonical Hungarian routes",
      "- /robots.txt is served by worker-robots-policy, contains Host: podiverzum.hu and explicitly allows OAI-SearchBot, PerplexityBot, DuckDuckBot and Bingbot",
      "- /llms.txt returns the short Podiverzum.hu AI-agent guidance and includes the Heti RSS, news sitemap and full sitemap URLs",
      "- /cd4aa0ff3daa6bff678ed60d1431affc45fcf9ef72ff14c90613492dc7c32f6a.txt returns only the IndexNow key; worker-indexnow-key header is preferred when served by Cloudflare",
    );
  }
